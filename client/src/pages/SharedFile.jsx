...
import { getSharedFile, verifyFilePassword, getGuestDownloadInfo, guestVerifyPassword } from '../api/fileApi';
...
const SharedFile = () => {
    ...
    const fetchFileInfo = async () => {
        ...
        try {
            const response = await getSharedFile(shortUrl);
            setFile(response.file);
            setIsGuest(false);
        } catch (err) {
            try {
                const guestResponse = await getGuestDownloadInfo(shortUrl);
    ...
    const handlePasswordVerify = async (e) => {
        ...
        try {
            let response;
            if (isGuest) {
                response = await guestVerifyPassword(shortUrl, password);
            } else {
                response = await verifyFilePassword(file.id, password);
            }
            setDownloadUrl(response.file.url);
        } catch (err) {
    ...
    if (error && !file) {
        return (
            <div className="min-h-screen relative flex items-center justify-center p-4">
                ...
                <div className="glass-light rounded-3xl shadow-2xl p-10 max-w-md w-full text-center animate-scale-in">
    ...
    return (
        <div className="min-h-screen relative flex items-center justify-center p-4">
            ...
            <div className="glass-light rounded-3xl shadow-2xl max-w-md w-full overflow-hidden animate-scale-in">
                {/* Header */}
                <div className="p-10 text-center group">
                    ...
                    <p className="text-gray-500">
                        {formatFileSize(file.size)} <span className="mx-2">•</span> {file.type}
                    </p>
                ...
                <div className="p-6 pt-0">
                    {isExpired ? (
                        <div className="glass rounded-xl p-6 text-center border border-red-200 bg-red-50/80">
                            ...
                            <p className="text-red-600 font-medium">This file has expired and is no longer available for download.</p>
                        </div>
