...
const Dashboard = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const images = userFiles.filter(f => f.type?.startsWith('image/')).length;
            const videos = userFiles.filter(f => f.type?.startsWith('video/')).length;
            ...
            setStats({ totalFiles: userFiles.length, totalSize, images, videos, documents });
        } catch (error) {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                {[
                    { label: 'Total Files', value: stats.totalFiles, icon: 'M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z', gradient: 'from-blue-500 to-indigo-600', shadow: 'shadow-blue-500/20' },
                    { label: 'Images', value: stats.images, icon: 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z', gradient: 'from-emerald-500 to-green-600', shadow: 'shadow-emerald-500/20' },
                    { label: 'Videos', value: stats.videos, icon: 'M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z', gradient: 'from-purple-500 to-violet-600', shadow: 'shadow-purple-500/20' },
                    { label: 'Storage', value: formatFileSize(stats.totalSize), icon: 'M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4', gradient: 'from-orange-500 to-amber-600', shadow: 'shadow-orange-500/20' },
                ].map((stat, index) => (
                    <div
                        key={stat.label}
                        ...
                        {files.map((file, index) => (
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="font-semibold text-gray-800 truncate group-hover:text-indigo-600 transition-colors">{file.name}</p>
