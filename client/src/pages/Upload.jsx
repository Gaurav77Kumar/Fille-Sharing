...
// ─────────────────────────────────────────────────────────
// Upload.jsx — What changed and why
//
// 1. updateStorageUsed called after successful upload
//    WHY: uploadFiles controller does $inc storageUsed on the server.
//    Without this call, the quota bar on Dashboard shows stale data
//    until the user refreshes the page.
//    Non-deduplicated files consume storage — deduplicated ones don't.
//
// 2. Success card shows per-file names + copy share link buttons
//    WHY: Old success card was useless — just "X files uploaded".
//    User had to navigate to My Files just to get the share link.
//    Now they can copy each link immediately after upload.
//    Uses file.originalName (renamed field) and file.slug (renamed field).
//
// 3. 413 quota error handled explicitly
//    WHY: Server returns 413 when storageUsed + batchSize > storageLimit.
//    Old code shows "Upload failed" — new code shows the specific message
//    the server sends including how much space is available.
//
// 4. file.type in selected files list is CORRECT — no change needed
//    WHY: files[] here contains browser File objects (before upload).
//    Browser File.type IS the MIME type string. This is not the server
//    model field — it's the native browser API. Leave it as file.type.
//    Only SERVER response fields need renaming (originalName, mimeType etc.)
// ─────────────────────────────────────────────────────────

const Upload = () => {
  const navigate = useNavigate();
  const { user, updateStorageUsed } = useAuth(); // FIX 1: get quota updater
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const newBytes = (response.files || [])
        .filter((f) => !f.deduplicated)
        .reduce((sum, f) => sum + (f.size || 0), 0);

      if (newBytes > 0 && user) {
        updateStorageUsed(user.storageUsed + newBytes);
      }
    } catch (err) {
      const status = err?.response?.status;
      if (status === 413) {
        setError(err.response?.data?.message || 'Storage quota exceeded. Delete some files to free up space.');
      } else {
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {uploadedFiles.length > 0 && (
        <div className="glass rounded-2xl overflow-hidden border border-emerald-200 animate-scale-in">
          ...
          <div className="p-6 bg-white/80 space-y-3">
            {/* Per-file share links */}
            {uploadedFiles.map((file, index) => (
              <div key={file._id} className="glass rounded-xl p-3 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">
                    ...
                    {file.deduplicated && (
                      <span className="ml-2 text-blue-500">· already existed, reused</span>
                    )}
                  </p>
                </div>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-xs rounded-lg font-medium transition-all whitespace-nowrap flex-shrink-0 ${
                    copiedIndex === index
