...
const GuestUpload = () => {
    ...
    const addFiles = (newFiles) => {
        ...
        if (files.length + newFiles.length > maxFiles) {
            setError(`Maximum ${maxFiles} files allowed for guest uploads`);
            return;
        }
    ...
    const getShareLink = (shortUrl) => {
        return `${window.location.origin}/share/${shortUrl}`;
    };

    const copyLink = (shortUrl, index) => {
        navigator.clipboard.writeText(getShareLink(shortUrl));
    ...
    return (
        <div className="min-h-screen relative overflow-hidden">
            ...
            <div className="max-w-2xl mx-auto px-4 py-12">
                ...
                {/* Info Banner */}
                <div className="glass-blue rounded-2xl p-5 mb-6 flex items-start gap-4 animate-fade-in-up">
                ...
                {uploadedFiles.length === 0 ? (
                    <>
                        ...
                        {/* Error Message */}
                        {error && (
                            <div className="mt-4 glass rounded-xl p-4 border border-red-200 bg-red-50/80 flex items-center gap-3 animate-shake">
                        ...
                        {/* Selected Files */}
                        {files.length > 0 && (
                            <div className="mt-6 glass-light rounded-2xl overflow-hidden animate-fade-in-up">
                                <div className="p-5 border-b border-white/20 flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500">{files.length} of 5 files</p>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <div className="h-2 w-24 bg-gray-200 rounded-full overflow-hidden">
                                            <div
                                                className="h-full bg-gradient-to-r from-emerald-500 to-blue-500 transition-all duration-500"
                                                style={{ width: `${(files.length / 5) * 100}%` }}
                                            />
                                        </div>
                                        <span className="text-sm font-medium text-gray-600">{files.length}/5</span>
                                    </div>
                                </div>
                        ...
                        {/* Progress Bar */}
                        {uploading && (
                            <div className="mt-4 glass-light rounded-2xl p-6 animate-fade-in">
                                ...
                                <div className="h-3 bg-gray-200 rounded-full overflow-hidden">
                                    <div
                                        className="h-full bg-gradient-to-r from-emerald-500 via-blue-500 to-indigo-500 transition-all duration-300 relative"
                    ...
                    <div className="glass rounded-2xl overflow-hidden border border-emerald-200 animate-scale-in">
                        <div className="p-8 bg-gradient-to-r from-emerald-500 to-blue-500 text-white text-center">
                            <div className="w-20 h-20 bg-white/20 rounded-2xl flex items-center justify-center mx-auto mb-4 animate-bounce-slow">
                            ...
                            <p className="text-white/80 text-lg">
                                Your files have been uploaded. Share the links below.
                            </p>
                        </div>
                        ...
                        <div className="p-6 bg-white/80">
                            <div className="glass rounded-xl p-4 mb-6 flex items-center gap-3 border border-orange-200 bg-orange-50/80">
                                ...
                                <p className="text-sm text-orange-700 font-medium">
                                    These files will expire in 24 hours
                                </p>
                            </div>
                                ...
                                {uploadedFiles.map((file, index) => (
                                    <div key={file._id} className="glass rounded-xl p-4">
                                        <div className="flex items-center justify-between mb-3">
                                            <p className="font-semibold text-gray-800 truncate flex-1">{file.name}</p>
                                            <span className="text-sm text-gray-500 ml-2">{formatFileSize(file.size)}</span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <input
                                                type="text"
                                                value={getShareLink(file.shortUrl)}
                                                readOnly
                                                className="flex-1 px-4 py-3 glass-input rounded-xl text-sm"
                                            />
                                            <button
                                                onClick={() => copyLink(file.shortUrl, index)}
