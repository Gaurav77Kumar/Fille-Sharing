...
import {  generateQRCode, sendFileEmail } from '../../api/fileApi';
...
const ShareModal = ({ file, onClose }) => {
  ...
  const [email, setEmail] = useState({ recipient: '', message: '' });
  const [emailQueued, setEmailQueued] = useState(false); 
  const [error, setError] = useState(''); 

  const shareLink = file.slug
    ? `${import.meta.env.VITE_BASE_URL || window.location.origin}/share/${file.slug}`
    : '';
  ...
  const handleSendEmail = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to queue email. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative glass-light rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-scale-in">
        ...
        {error && (
          <div className="mx-6 mt-4 px-4 py-3 rounded-xl text-sm font-medium flex items-center gap-2 bg-red-50 border border-red-200 text-red-700">
        ...
        <div className="p-6">
          ...
          {activeTab === 'link' && (
            <div className="space-y-4">
              ...
              {!shareLink ? (
                <div className="glass rounded-xl p-4 border border-red-200 bg-red-50/80 text-sm text-red-700 text-center">
                  Share link unavailable — file slug is missing. Try refreshing My Files.
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  {file.isPasswordProtected && (
                    <div className="glass rounded-xl p-4 border border-amber-200 bg-amber-50/80 flex items-center gap-3">
                      ...
                      <p className="text-sm text-amber-700 font-medium">
                        Password required to download this file
                      </p>
                    </div>
                  )}

                  {file.expiresAt && new Date(file.expiresAt) > new Date() && (
                    <div className="glass rounded-xl p-4 border border-orange-200 bg-orange-50/80 flex items-center gap-3">
                      <svg className="w-5 h-5 text-orange-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      <p className="text-sm text-orange-700 font-medium">
                        Link expires {new Date(file.expiresAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-4 text-center">
              {!qrCode ? (
                ...
                <div className="space-y-4">
                  ...
                  <a
                    href={qrCode}
                    download={`${file.originalName}-qr.png`}
          ...
          {activeTab === 'email' && (
            <div>
              {emailQueued ? (
                // Email goes into BullMQ queue and is sent in the background.
                // Server returns 200 immediately — email may arrive in a few seconds.
                <div className="text-center py-6">
                  ...
                  <h3 className="font-bold text-gray-800 text-xl mb-2">Email Queued!</h3>
                  <p className="text-gray-500 mb-1">The email is being sent in the background.</p>
                  <p className="text-sm text-gray-400 mb-6">It should arrive within a few seconds.</p>
                  <button
                    onClick={() => setEmailQueued(false)}
