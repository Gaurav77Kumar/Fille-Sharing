...
const FileDetailsModal = ({ file, onClose, onDelete, onShare, onStatusChange, onUpdate }) => {
    ...
    const [expiry, setExpiry] = useState({
        hasExpiry: file.hasExpiry || false,
    ...
    const handleExpiryUpdate = async () => {
        setLoading(true);
        try {
            const response = await updateFileExpiry(
                file._id,
                expiry.hasExpiry,
                expiry.hasExpiry ? new Date(expiry.expiresAt).toISOString() : null
            );
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-500">
                                    {formatFileSize(file.size)} <span className="mx-1">•</span> {file.type}
                                </p>
                            </div>
                        </div>
                        <button
                ...
                <div className="p-6 overflow-y-auto max-h-[50vh]">
                    {activeTab === 'details' && (
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="glass rounded-xl p-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className={`w-2 h-2 rounded-full ${file.status === 'active' && !isExpired ? 'bg-emerald-500' : 'bg-gray-400'}`}></div>
                                ...
                                <div className="glass rounded-xl p-4">
                                    ...
                                    <p className="font-semibold text-gray-800 text-xl">{file.downloadContent || 0}</p>
                                </div>
                                <div className="glass rounded-xl p-4">
                            ...
                            {file.hasExpiry && (
                                <div className={`glass rounded-xl p-4 border ${isExpired ? 'border-red-200 bg-red-50/80' : 'border-orange-200 bg-orange-50/80'}`}>
                            ...
                            <div className="glass rounded-xl p-5">
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <label className="relative inline-flex items-center cursor-pointer">
                                        <input
                                            type="checkbox"
                                            checked={expiry.hasExpiry}
                                            onChange={(e) => setExpiry(prev => ({
                                                ...prev,
                                ...
                                {expiry.hasExpiry && (
                                    <div className="space-y-3">
                                        <input
                                            ...
                                            onChange={(e) => setExpiry(prev => ({ ...prev, expiresAt: e.target.value }))}
                                            className="w-full px-4 py-3 glass-input rounded-xl focus:ring-2 focus:ring-indigo-500/30"
                                        />
                                        <button
                                ...
                                {!expiry.hasExpiry && file.hasExpiry && (
                                    <button
                                        onClick={handleExpiryUpdate}
                                        disabled={loading}
                                        className="w-full py-3 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-xl font-semibold hover:from-red-600 hover:to-red-700 disabled:opacity-50 transition-all"
